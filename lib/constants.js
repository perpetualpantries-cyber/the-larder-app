export const GROUPS = [
  { label: "Keeper", items: ["keeper", "brief"] },
...
export const CATS = {
  ...
  brief: { label: "Morning Brief", short: "Brief", blurb: "A daily summary of what's open and what's new, written by Keeper." },
...
GROUPS.forEach((g) => {
  g.items.forEach((id) => {
    if (id !== "tasks" && id !== "keeper" && id !== "brief") TASK_PROJECTS.push({ id, label: CATS[id].short });
  });
});
